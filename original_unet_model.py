...
class unet2D(nn.Module):
    def __init__(self, layers, num_classes=15, num_scale=4, weight_std=False):
        ...
        self.controller_trilinear = nn.Conv2d(
            256, 162, kernel_size=1, stride=1, padding=0
        )

        self.cls_emb = nn.Parameter(
            torch.randn(1, num_classes, 32 + 32 + 64 + 128 + 256 + 256)
        )
        self.sls_emb = nn.Parameter(
            torch.randn(1, num_scale, 32 + 32 + 64 + 128 + 256 + 256)
        )

    def _make_layer(
        self, block, inplanes, planes, blocks, stride=(1, 1), dilation=1, multi_grid=1
    ):
        downsample = None
